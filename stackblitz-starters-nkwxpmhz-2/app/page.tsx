...
/* ===================== HOME TAB (single toggle + slider, NO boxes) ===================== */
function HomeTab() {
  ...
  return (
    <div>
      <div className="text-xs text-gray-500 mb-1">v-ToggleOnly</div>
      <h1 className="text-xl font-bold mb-2">Budget</h1>
      <p className="text-gray-600 mb-4">
        Set your budget <span className="font-semibold">per {period}</span>.
      </p>
...
function DealsTab() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch(`/api/deals?zip=${encodeURIComponent(zip)}`);
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-3">
        ...
        <button onClick={load} className="px-3 py-1 rounded bg-blue-600 text-white">{loading ? "Loading..." : "Refresh"}</button>
      </div>
      <ul className="space-y-2">
        {deals.map((d, i) => (
          <li key={d.id ?? i} className="border p-2 rounded bg-green-50 flex justify-between">
            <span>{d.title} – {d.store.toUpperCase()}</span>
